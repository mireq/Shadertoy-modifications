...
float doubleSpiral = clamp((cos(iTime / 7.0) + 1.0) * 2.0, 0.0, 1.0);
float holes = smoothstep(0.45, 0.55, abs(fract((iTime - 8.0) / 17.0) * 2.0 - 1.0));
float raised = smoothstep(0.7, 0.8, abs(fract(iTime / 19.0) * 2.0 - 1.0));
float ridges = smoothstep(0.7, 0.8, abs(fract((iTime + 10.0) / 23.0) * 2.0 - 1.0));
float vert_lines = smoothstep(0.7, 0.8, abs(fract((iTime + 5.0) / 27.0) * 2.0 - 1.0));

float shapeDuration = 11.0;
float transitionDuration = 0.1;
float shapeTime = iTime + 1.0;

float fadeInValue = clamp(fract(shapeTime / shapeDuration) / transitionDuration, 0.0, 1.0);
float fadeOutValue = clamp(1.0 - (fract(shapeTime / shapeDuration) / transitionDuration), 0.0, 1.0);
float shapeSquare = int(shapeTime / shapeDuration) % 3 == 2 ? fadeInValue : (int(shapeTime / shapeDuration) % 3 == 0 ? fadeOutValue : 0.0);
float shapeCircle = int(shapeTime / shapeDuration) % 3 == 1 ? fadeInValue : (int(shapeTime / shapeDuration) % 3 == 2 ? fadeOutValue : 0.0);
float shapeHexagon = int(shapeTime / shapeDuration) % 3 == 0 ? fadeInValue : (int(shapeTime / shapeDuration) % 3 == 1 ? fadeOutValue : 0.0);



vec2 rDim = mix(vec2(1, 2.*.8660254), vec2(1, 2), shapeSquare);
...
vec4 blocks(vec3 q3){
    ...
    for(int i = min(0, iFrame); i<4; i++){
        ...
        //#endif
        float di2D = mix(mix(sBoxS(p, l/2. - .0035, .02), length(p) - minSc/1.732 + .0035, shapeCircle), sHexS(p, minSc/1.732 - .0035, .015), shapeHexagon);
        
        
        
        // Boring out the objects.
        di2D = mix(di2D, max(di2D, -(di2D + minSc/3.)), holes);
        ...
        di = mix(di, di + di2D*.5*tempR, raised);
        
        di += mix(di, smoothstep(-.5, .5, sin(di2D/minSc*6.2831*3.))*.01, ridges);
...
float map(vec3 p){
    ...
    float sc2 = 0.75 * doubleSpiral;
    float r2 = min(length(z2.xy - vec2(sc2, 0)), length(z2.xy - vec2(-sc2, 0)));
    tempR = (doubleSpiral * min(tempR, r2)) + ((1.0 - doubleSpiral) * tempR);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    scale = mix(scale*vec2(2./1.732, 1), scale, shapeSquare);
	...
	if(t < FAR){
        ...
        if(svObjID<.5){
            ...
            float aNum = mix(4.0, 6.0, shapeHexagon);
            vec2 z = mix(svP, rot2(3.14159/aNum)*svP, shapeHexagon);
            float a = mod(atan(z.x, z.y), 6.2831)/6.2831;
            a = (floor(a*aNum) + .5)/aNum;
            z = mix(z, z*rot2(a*6.2831), vert_lines);
            texCol = mix(texCol, mix(texCol, vec3(0), (1. - smoothstep(0., sf, abs(z.x) - .001))*.95), vert_lines);
