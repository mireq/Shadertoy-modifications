...
const float color_duration = 5.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 p = (fragCoord - iResolution.xy*.5)/min(1700., iResolution.y);
    ...
    float color_scheme[3] = float[3](
        smooth_fade(time, color_duration * 0.0, color_duration * 0.0 + color_duration, color_fade_time, color_duration * 3.0),
        smooth_fade(time, color_duration * 1.0, color_duration * 1.0 + color_duration, color_fade_time, color_duration * 3.0),
        smooth_fade(time, color_duration * 2.0, color_duration * 2.0 + color_duration, color_fade_time, color_duration * 3.0)
    );
    ...
    if(t < FAR){
        ...
        //#endif
        vec3 col = vec3(.6) * color_scheme[0] + vec3(.1) * color_scheme[1] + vec3(.9, .2, .4) * color_scheme[2];
        ...
        if(svObjID == 1.) { // Smaller hexagon cogs.
            ...
            //col = vec3(.6); // Chrome.
            //#endif
            col = vec3(1, .5, .2)*.7 * color_scheme[0] + vec3(.6) * (color_scheme[1] + color_scheme[2]);
         }
        if(svObjID == 2.){ // Larger hexagon and pentagon cogs.
            ...
            //col = vec3(.6); // Chrome.
            //#endif
            col = vec3(1, .65, .3)*.7* color_scheme[0] + vec3(.6) * (color_scheme[1] + color_scheme[2]);
        }
        if(svObjID == 3.) { // Back plane.
            ...
            //#endif
            col = vec3(1, .7, .4)*.045 * color_scheme[0] + vec3(.7)*.045 * color_scheme[1] + vec3(.6, .7, 1)*.045 * color_scheme[2];
        }
